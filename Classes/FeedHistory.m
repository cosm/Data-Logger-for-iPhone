...
@implementation FeedHistory
@synthesize graphView, image, mapSwitch, titleLabel;
...
- (void)viewDidLoad {
	...
	UIImage *blockedMenuItem = [UIImage imageWithContentsOfFile: [[NSBundle mainBundle] pathForResource:@"nevercallanythingthis" ofType:@"png"]];

	[self.navigationItem.rightBarButtonItem setImage:blockedMenuItem];
	

	//[self.navigationItem.rightBarButtonItem ]; 
	//self.navigationItem.rightBarButtonItem = ;
	//self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemPlay target:self action:@selector(turnOnEditing)];
...
- (void)displayImage {
	...
	NSURL *getUrlImage = [NSURL URLWithString:@"http://www.pachube.com/feeds/2474/datastreams/0/history.png?w=288&h=277"];
	
	[graphView setHidden:NO];
	[graphView setEnabled:YES];
	image = [UIImage imageWithData: [NSData dataWithContentsOfURL:getUrlImage]];
